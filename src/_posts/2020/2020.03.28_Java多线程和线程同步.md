...
## 多线程的安全和同步
当然，上面解释只是一个简单的描述，回顾前面的内容，我们知道每个线程的工作内存是线程私有内存。而在多线程场景下主内存中的共享变量是多个线程共享的，线程在执行时会读取主内存的共享变量，存在线程私有内存中，由于线程间无法互相访问对方的工作内存，所一旦某个线程意外阻塞了，其将数据写回主内存的时候就会写入`过时的数据`，此刻主内存中共享变量是不安全的。
